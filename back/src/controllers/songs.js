...
songsRouter.post("", authorizationMiddleware, async (request, response) => {
  ...
  try {
    ...
    if (!match || match[2].length !== 11)
      return response.status(400).send({ error: "Not a youtube URL" });

    if (await Song.findOne({ url }))
      return response.status(400).send({ error: "Song already exists" });

    if (album.length !== 24)
      return response.status(400).send({ error: "Album does not exist" });

    const fatherAlbum = await Album.findById(album);
    if (!fatherAlbum)
      return response.status(400).send({ error: "Album does not exist" });

    if (fatherAlbum.artist != userId)
      return response
        .status(400)
        .send({ error: "Album does not belong to this artist" });
...
songsRouter.get("/:songId", async (request, response) => {
  try {
    ...
    if (song) return response.send(song);
      ...
      return response.status(404).json({ message: "Could not find the song" });
  } catch (error) {
    return response
      .status(500)
...
songsRouter.get("/fromAlbum/:albumId", async (request, response) => {
  try {
    ...
    if (songs) {
      ...
      return response
        .status(404)
        .json({ message: "Album does not exist or album has no songs" });
  } catch (error) {
    return response
      .status(500)
      .json({ message: "Something went wrong with the server", error });
